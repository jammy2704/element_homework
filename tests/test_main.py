...
def test_n_greater_than_array_size():
    response = client.post("/top_n", json={"n": '15', "arr": [10, 30, 20, 21, 11, 22, 33, 44, 15, 33, 10, 30]})
    assert response.status_code == 400
    assert response.json()['detail'] == 'length of array should be more than n'

def test_valid_input():
    response = client.post("/top_n", json={"n": '5', "arr": [10, 30, 20, 21, 11, 22, 33, 44, 15, 33, 10, 30]})
    assert response.status_code == 200
    assert response.json() == {"arr": [44, 33, 33, 30, 30]}

def test_n_equals_array_size():
    response = client.post("/top_n", json={"n": '12', "arr": [10, 30, 20, 21, 11, 22, 33, 44, 15, 33, 10, 30]})
    assert response.status_code == 200
